...
fn hex_to_base64(hex_input: Vec<u8>) -> Result<Vec<u8>, String> 
{
	if hex_input.len() <= 0
	{
...
fn hex_string_to_bytes(hex_string: String) -> Result<Vec<u8>, String>
{
	if hex_string.len() <= 0 
	{
		return Err("Invalid hex vector. Length of hex vector is less than or equal to zero.".to_string());
...
fn main() 
{
	//let ascii_string = String::from("I'm killing your brain like a poisonous mushroom");
	...
	let hex_input: Vec<u8> = hex_string_to_bytes(hex_string).unwrap();

	//let mut hex_input: Vec<u8> = hex_string.into_bytes();

	match hex_to_base64(hex_input)
	{
